...
# Import "forms.py", i.e, another python file into this python file
from forms import PostForm, UserForm, LoginForm, PasswordForm, SearchForm

#create flask instance
app = Flask(__name__)

# Add a Text Editor for better blog content writing
...
# To upload files to a certain place 
...
class Users(db.Model, UserMixin):
    ...
    about_user = db.Column(db.Text(500), nullable=True)
...
# Pass Stuff to Navbar.html because navbar.html doesn't have any form 
# being sent to it to use search function
@app.context_processor
def base():                           # it is called base bcuz we use base function to connect navbar and bootstrap to all the pages
    form = SearchForm()
...
def search():
    ...
    if form.validate_on_submit():        
        ...
        post.searched_for = form.searched_for.data

        # Query the Database
        posts = posts.filter(Posts.content.like('%' + post.searched_for + '%'))
        posts = posts.order_by(Posts.title).all()
        return render_template("search.html", form = form, searched_for = post.searched_for, posts = posts)
